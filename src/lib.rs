...
impl Drop for AllocDrop {
	...
	fn drop(&mut self) {
		if self.size_bytes > 0 {
			unsafe { alloc::alloc::dealloc(self.ptr, alloc::alloc::Layout::from_size_align_unchecked(self.size_bytes, self.align)) }
		}
	}
}
...
impl<T, A: Alignment> AVec<T, A> {
	...
	pub fn new(align: usize) -> Self {
		unsafe {
			Self {
				buf: ARawVec::new_unchecked(A::new(align, align_of::<T>()).alignment(align_of::<T>())),
				len: 0,
			}
		}
	}
	...
	pub fn with_capacity(align: usize, capacity: usize) -> Self {
		unsafe {
			Self {
				buf: ARawVec::with_capacity_unchecked(capacity, A::new(align, align_of::<T>()).alignment(align_of::<T>())),
				len: 0,
			}
		}
	}
...
mod serde {
	...
	impl<'de, T: Deserialize<'de>, const N: usize> Deserialize<'de> for ABox<T, ConstAlign<N>> {
		...
		{
			Ok(ABox::<T, ConstAlign<N>>::new(N, T::deserialize(deserializer)?))
		}
	}
	...
	impl<'de, T: Deserialize<'de>, const N: usize> Deserialize<'de> for AVec<T, ConstAlign<N>> {
			...
			D: ::serde::Deserializer<'de>,
		{
			...
			impl<'de, T: Deserialize<'de>, const N: usize> ::serde::de::Visitor<'de> for AVecVisitor<T, N> {
				...
				{
					let mut vec = AVec::<T, ConstAlign<N>>::with_capacity(N, cautious::<T>(seq.size_hint()));
			...
			deserializer.deserialize_seq(AVecVisitor { _marker: PhantomData })
		}
	}
...
/// ```rust
/// use aligned_vec::{CACHELINE_ALIGN, avec};
...
mod serde_tests {
	...
	fn can_limit_deserialization_size() {
		// Malformed serialized data indicating a sequence of length u32::MAX.
		let ser = vec![253, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 1, 1, 1, 1, 1, 1, 1, 1];
